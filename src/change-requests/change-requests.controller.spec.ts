...
describe('ChangeRequestsController', () => {
  ...
  describe('create', () => {
    it('changeRequestsService.createChangeRequest를 1회 호출하고 반환값을 그대로 전달한다', async () => {
      const dto: CreateChangeRequestDto = { reason: '납기일 변경', changes: {} as any };
      ...
      expect(mockService.createChangeRequest).toHaveBeenCalledWith(10, dto, 'user-abc');
      expect(result).toEqual(expected);
    });

    it('req.userId가 null이면 빈 문자열로 전달된다', async () => {
      const dto: CreateChangeRequestDto = { reason: '사유', changes: {} as any };
  ...
  describe('approve', () => {
    it('changeRequestsService.approveChangeRequest를 1회 호출하고 반환값을 전달한다', async () => {
      ...
      expect(mockService.approveChangeRequest).toHaveBeenCalledWith(10, 5, dto, 'user-abc');
      expect(result).toEqual(expected);
    });
    ...
    it('req.userId가 null이면 빈 문자열로 전달된다', async () => {
      ...
      expect(mockService.approveChangeRequest).toHaveBeenCalledWith(10, 5, dto, '');
    });
  });
  ...
  describe('reject', () => {
    it('changeRequestsService.rejectChangeRequest를 1회 호출하고 반환값을 전달한다', async () => {
      ...
      expect(mockService.rejectChangeRequest).toHaveBeenCalledWith(10, 5, dto, 'user-abc');
      expect(result).toEqual(expected);
    });
  });
});
