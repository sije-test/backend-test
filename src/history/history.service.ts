...
export class HistoryService {
  ...
  async getHistory(orderId: number) {
    ...
    return this.repo.findVersionsByOrder(orderId);
  }

  /** 특정 버전의 스냅샷을 반환한다. 버전이 존재하지 않으면 404를 던진다. */
  async getVersionSnapshot(orderId: number, version: number) {
    await this.ordersService.findOrderById(orderId);
    const snapshot = await this.repo.findVersion(orderId, version);
    if (!snapshot) {
      this.logger.warn(
  ...
  async getSnapshotAtTimestamp(orderId: number, timestamp: string) {
    ...
    const snapshot = await this.repo.findLatestVersionBefore(orderId, parsed);
    if (!snapshot) {
      this.logger.warn(
  ...
  async getStatusHistory(orderId: number) {
    await this.ordersService.findOrderById(orderId);
    return this.repo.findStatusLogsByOrder(orderId);
  }
}
